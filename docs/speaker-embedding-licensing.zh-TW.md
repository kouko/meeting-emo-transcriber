...
**唯一殘留軟點**:官網只明寫 metadata 是 CC-BY-SA,**音檔本身授權未明講**。
→ **出貨不需要為此做任何事**:控制閘是模型權重(Apache-2.0,已確認、無歧義);音檔授權只在
「再散布資料集」時才相關,而 metr 只用權重、不散布資料集。就算要它咬到,還得疊加「資料授權傳染到
權重」這個最弱理論(Getty 偏否定),複合機率極低。寄信確認音檔授權純屬選擇性 belt-and-suspenders,
**非上架必要條件**。
...
- [ ] **[需律師]** 生物辨識隱私(BIPA/GDPR)合規 — 聲紋是生物特徵,與選模型無關但可能是最大實際曝險
- [ ] 工程:sherpa-onnx 接 ERes2Net-large ONNX、`--rebuild` 重算、門檻校準

> ~~寄信問維護者確認資料集音檔授權~~ — **非必要**。模型權重 Apache-2.0 是控制閘且已確認;
> 音檔授權僅在散布資料集時相關,metr 不散布資料集。僅在想對音檔來源做書面背書時才選擇性進行。
