...
func newInitCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "init",
		Short: "Initialize working directory (creates speakers/ + speakers/_metr/ + config.yaml template)",
		RunE: func(cmd *cobra.Command, args []string) error {
			dirs := []string{"speakers", "speakers/_metr", "output"}
			for _, d := range dirs {
